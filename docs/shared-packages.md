...
P1B declares the already-vendored `@diary/domain` artifact as a file dependency for `calendarDateInTimezone` and `deriveQuickTitle`. It does not repack or change any artifact or provenance. Quick Diary uses the existing generated by-date/create operations and shared request/response schemas.
