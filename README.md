...
`diary-app` is a standalone Expo SDK 57 Android client for the existing diary-v3 API. Trade Basic includes registration, account security, full preferences with three locales/themes, safe native Markdown reading, native authentication, searchable Timeline, monthly Calendar, diary Review Queue and Detail, native Review authoring with encrypted local drafts, and Quick Diary with encrypted durable drafts and explicit create/append saves.

**Delivery target:** the complete diary-v3 product in the native app, including Portfolio, Company research, all Tools, Partners/Agent workflows, Articles and administration. Follow the [full launch roadmap](docs/launch-roadmap.md) and [feature parity matrix](docs/feature-parity.md). The features described above are the implemented subset; beta is an intermediate acceptance activity, not the final scope.

Implementation is specified in the [full-app PRD](.scratch/diary-app-full-parity/PRD.md) and [70-ticket local issue tracker](.scratch/diary-app-full-parity/ISSUES.md), with dependencies, requirement coverage and acceptance criteria. F0 engineering foundation is complete: the five-section shell, shared-package alignment and acceptance harness are delivered. F1 (tickets 06–09) is also complete at host, disposable API and Android development-emulator layers. See [F1 evidence](docs/evidence/f1/acceptance.md) and the [F1 runbook](docs/f1-acceptance.md), alongside [F0 evidence](docs/evidence/f0/acceptance.md). F2 has not started; full-product release acceptance remains open.
